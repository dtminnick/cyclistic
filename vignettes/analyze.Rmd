...
Create a modern, minimal custom theme for visuals.

```{r create_ggplot_theme}
cyclistic_theme <- theme_light() +
...
```{r plot_trips_by_member_and_ride_type}
...
Plot duration using kernal density plot, split by member type and category of day.

```{r plot_trips_by_member_type_and_duration_split_density}
ggplot(trip, 
       aes(x = trip_duration_in_minutes, y = member_type, fill = category_of_day)) +
...
ggsave(filename = "trips_by_member_type_and_duration_split_density.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
business <- st_read("C:/R/Packages/cyclistic/inst/extdata/chicago/business/geo_export_f4ec6c23-2aae-4c59-8822-17d79b57d91f.shp")
```
...
Start by generating a data frame that contains the trip count for each starting station in descending order of count.  Include relative frequency for each station.
...
ggplot() +
  ...
  labs(title = "Geographic Density of Trip Starts",
       subtitle = "Trip starts are concentrated in and around Chicago's central business district.",
       x = "Longitude",
       y = "Latitude") + 
  cyclistic_theme

ggsave(filename = "trip_starts.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
Do the same for ending stations.
...
ggplot() +
  ...
  csf + 
  geom_sf(data = business, fill = "grey", alpha = 0.4) +
  labs(title = "Geographic Density of Trip Ends",
       subtitle = "Trip ends are also concentrated in and around Chicago's central business district.",
       x = "Longitude",
       y = "Latitude") + 
  cyclistic_theme

ggsave(filename = "trip_ends.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
### Plot Trip Density Using Stadia Maps

Code below uses stamen map tiles served by Stadia Maps to plot trip density.  I am using Stadia Maps to see if the plots are more easily read (versus shape files). 

Note that, to use Stadia Maps, you will need to setup an account and generate an API key. The code below using the keyring package to retrieve my personal API key from my local machine.

Get Stadia map for Chicago

```{r get_api_key}
register_stadiamaps(key = keyring::key_get(service = "Stadia",
                                           username = "donnie.minnick@gmail.com"))

chicago_sm <- get_stadiamap(bbox = c(bb[1,1], bb[2,1], 
                                     bb[1,2], bb[2,2]),
                            maptype = "stamen_toner_lite")
```

```{r stadia_plot_trip_starts}
ggmap(chicago_sm) + 
  geom_point(data = trip_starts, 
             aes(x = start_longitude, 
                 y = start_latitude,
...
ggsave(filename = "trip_starts_stadia.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
ggmap(chicago_sm) + 
  ...
  scale_color_identity() +
  labs(title = "Geographic Density of Trip Ends",
       subtitle = "Trip ends are concentrated in and around Chicago's central business district.",
       x = "Longitude",
       y = "Latitude") + 
  cyclistic_theme

ggsave(filename = "trip_ends_stadia.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
ggplot() +
  ...
  labs(title = "Existing Bike Routes in Chicago",
    subtitle = "....",
    x = "Longitude",
    y = "Latitude") + 
  scale_fill_manual(values = cbPalette) + 
...
ggplot() +
  ...
  labs(title = "Existing Parks in Chicago.",
    subtitle = "....",
    x = "Longitude",
    y = "Latitude") + 
  scale_fill_manual(values = cbPalette) + 
...
Chicago has `r format(nrow(landmarks), big.mark = ",", scientific = FALSE)` local landmarks throughout the city.

Plot landmarks.

```{r plot_local_landmarks}
ggplot() +
  geom_sf(data = communities, fill = "white") + 
  geom_sf(data = landmarks, fill = "#E69F00") + 
  labs(title = "Existing Local Landmarks in Chicago.",
    subtitle = "....",
    x = "Longitude",
    y = "Latitude") + 
  scale_fill_manual(values = cbPalette) + 
...
ggsave(filename = "local_landmarks.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
Chicago has `r format(nrow(national_landmarks), big.mark = ",", scientific = FALSE)` national landmarks throughout the city.

Plot national landmarks.

```{r plot_national_landmarks}
ggplot() +
  geom_sf(data = communities, fill = "white") + 
  geom_sf(data = national_landmarks, fill = "#E69F00") + 
  labs(title = "National Landmarks in Chicago.",
    subtitle = "....",
    x = "Longitude",
    y = "Latitude") + 
  scale_fill_manual(values = cbPalette) + 
...
ggsave(filename = "national_landmarks.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
