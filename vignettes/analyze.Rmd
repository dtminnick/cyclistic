...
### Proportion of Trips by Customer Type and Category of Day

Establish the proportion of trips taken by casual riders and members by category of day (weekday versus weekend).

```{r trips_by_member_type_category_of_day}
trips_by_member_type_and_category_of_day <- trip %>%
  group_by(category_of_day, member_type) %>%
  summarise(trip_count = n()) %>%
  mutate(percent = round(trip_count / sum(trip_count), 2)) %>%
  bind_rows(summarise(., across(where(is.numeric), sum),
                       across(where(is.character), ~ "Combined")))

kable(trips_by_member_type_and_category_of_day,
      col.names = c("Category of Day", "Member Type", "Trip Count", "Percent"),
      caption = "Trips by Member Type and Category of Day",
      format.args = list(big.mark = ","),
      align = c("l", "l", "r", "r"))
```
...
wday_trips <- trip %>% filter(category_of_day == "Workweek")

wend_trips <- trip %>% filter(category_of_day == "Weekend")

wday_hourly_counts <- hourly_counts %>%
  rowwise() %>%
  mutate(active_trip_count = count_active_trips(hour_of_day, wday_trips$start_hour_of_day, wday_trips$end_hour_of_day))

kable(wday_hourly_counts,
...
ggplot(wday_hourly_counts, aes(x = hour_of_day, y = active_trip_count)) +
  geom_bar(stat = "identity", fill = cbPalette[2]) + 
...
ggsave(filename = "wday_trips_by_hour_interval.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
```{r wend_trips_by_hour_interval, messages = FALSE}
wend_trips <- trip %>% filter(category_of_day == "Weekend")

wend_hourly_counts <- hourly_counts %>%
  rowwise() %>%
  mutate(active_trip_count = count_active_trips(hour_of_day, wend_trips$start_hour_of_day, wend_trips$end_hour_of_day))

kable(wend_hourly_counts,
      col.names = c("Hour", "Active Trip Count"),
      caption = "Active Trips by Hour Interval",
      format.args = list(big.mark = ","),
      align = c("l", "r"))
...
```{r plot_trips_by_hour_interval, message = FALSE}
ggplot(wend_hourly_counts, aes(x = hour_of_day, y = active_trip_count)) +
  geom_bar(stat = "identity", fill = cbPalette[2]) + 
  geom_smooth(se = FALSE, colour = cbPalette[1]) +
  labs(title = "Active Trips by Hour Interval",
   subtitle = "Trips peak between the hours of 7:00-9:00 am and 4:00-6:00 pm.",
...
ggsave(filename = "wend_trips_by_hour_interval.png",
       path = "C:/R/Packages/cyclistic/inst/extdata/plots/",
       width = 7,
       height = 4,
       dpi = 600)
...
ggplot() +
  ...
  theme(axis.title.x = element_blank(),
        ...
        plot.background = element_rect(fill = "#FBFAF7"),
        strip.text.x = element_blank()) + 
  csf + 
  geom_sf(data = business, fill = "grey", alpha = 0.5)
...
ggplot() +
  ...
  geom_sf(data = routes, color = "#4F81BD") + 
    theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.y = element_blank(),
...
ggplot() +
  geom_sf(data = communities, fill = "white") + 
  geom_sf(data = parks, fill = "#4F81BD") + 
    theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.y = element_blank(),
...
ggplot() +
  geom_sf(data = communities, fill = "white") + 
  geom_sf(data = landmarks, fill = "#4F81BD") + 
    geom_sf(data = national_landmarks, fill = "#4F81BD") + 
    theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.y = element_blank(),
